...
class TrainingArguments:
    ...
    train_data_ratio: Optional[float] = field(
        default=1.0,
        metadata={
            "help": "The ratio for training data. This is used when we want to use only a part of the training data to train the model."
        },
    )
    ...
    use_tb: bool = field(
        default=False,
        metadata={
            "help": 'If use TempBalance for learning rate scheduling.'
        },
    )

    after_warm: str = field(
        default="False",
        metadata={
            "help": "If use TempBalance after warmup.",
            "choices": ["False", "True"],
        },
    )

    tb_late_start: bool = field(
        default=False,
        metadata={
            "help": "Whether start using TB after the first epoch."
        },
    )

    hyperparam_to_tune: str = field(
        default="lr",
        metadata={
            "help": "Hyperparameter to tune using TB. Can choose from {'lr', 'wd', 'hybrid'}",
            "choices": ["lr", "wd", "hybrid"],
        },
    )

    fix_tb_metric: str = field(
        default="False",
        metadata={
            "help": "If use fixed tb metric for lr scheduling during entire training."
        },
    )

    temp_balance_lr: str = field(
        default='tb_linear_map',
        metadata={
            "help": "TempBalance scheduling method for learning rate."
        },
    )

    temp_balance_wd: str = field(
        default='tb_linear_map',
        metadata={
            "help": "TempBalance scheduling method for weight decay."
        },
    )

    schedule_func: str = field(
        default='linear_map',
        metadata={
            "help": "Scheduling function when using block_tb."
        },
    )

    sigmoid_min: Optional[float] = field(
        default=2,
    )

    sigmoid_max: Optional[float] = field(
        default=1,
    )

    exp_temp: Optional[float] = field(
        default=10,
    )

    tb_lr_normalize: str = field(
        default='False',
        metadata={
            "help": "If normalize the learning rate of each layer after rescheduling,"
        },
    )
    ...
    metric: str = field(
        default='alpha',
        metadata={
            "help": "Metric to use to do ESD analysis"
        },
    )
    ...
    esd_type: str = field(
        ...
        metadata={
            "choices": ["full", "cul"],
            "help": "Which type of ESD to use: full matrix ESD, or cumulative matrix ESD.",
        },
    )

    remove_last_layer: str = field(
        default='False',
        metadata={
            "help": "If remove the last layer when doing layer-wise parameter tuning."
        },
    )

    esd_interval: int = field(
        default=100,
        metadata={
            "help": "The interval(step) for ESD calculation."
        },
    )

    tb_interval: int = field(
        default=100,
        metadata={
            "help": "The interval(step) for TempBalance scheduling."
        },
    )

    fix_fingers: str = field(
        default="xmin_mid",
        metadata={
            "help": "Fix finger method."
        },
    )

    xmin_pos: float = field(
        default=2,
        metadata={
            "help": "Position of smallest eigenvalue for ht_alpha fitting."
        },
    )

    filter_zeros: str = field(
        default='False',
        metadata={
            "help": "If filter out eigenvalues that are too small."
        },
    )
    ...
    lr_min_ratio: float = field(
        default=0.5,
        metadata={
            "help": "Lower scale of rescheduled lr."
        },
    )

    lr_slope: float = field(
        default=1.0,
        metadata={
            "help": "Upper scale of rescheduled lr."
        },
    )

    wd_min_ratio: float = field(
        default=0.5,
        metadata={
            "help": "Lower scale of rescheduled wd."
        },
    )

    wd_slope: float = field(
        default=1.0,
        metadata={
            "help": "Upper scale of rescheduled wd."
        },
    )
    ...
    batchnorm: str = field(
        default='False',
        metadata={
            "help": "If schedule batchnorm layers with tempbalance."
        }
    )
    ...
    tb_unit: str = field(
        default='block',
        metadata={
            "help": "The setting of unit of lr_tuning in tb_layer_block method."
        },
    )

    tb_window_idx: int = field(
        default=None,
        metadata={
            "help": "In tb_unit sliding window, select which block has high learning rate."
        },
    )

    tb_window_scale: float = field(
        default=None,
        metadata={
            "help": "In tb_unit sliding window, select the upscale magnitude of learning rate of the chosen block."
        },
    )

    record_hessian: bool = field(
        default=False,
        metadata={
            "help": "If record hessian data."
        },
    )

    hessian_count: int = field(
        default=20,
        metadata={
            "help": "Number of hessian stats to store throughout training."
        }
    )

    hessian_data: str = field(
        default="train",
        metadata={
            "help": "Which dataset to use when calculating hessian.",
            "choices": ["train", "test"],
        },
    )

    mini_hessian_batch_size: int = field(
        default=200,
    )

    hessian_batch_size: int = field(
        default=2000,
        metadata={
            "help": "Input batch size for hessian."
        },
    )

    record_grad_norm: bool = field(
        default=False,
        metadata={
            "help": "If recording the gradient norm."
        },
    )
    ...
    rho: float = field(
        default=0.05,
        metadata={
            "help": "Hyperparameter rho for the SAM optimizer."
        },
    )
    ...
    lora_esd: str = field(
        default="False",
        metadata={
            "help": "If calculating the ESD of LoRA layers."
        },
    )

    lora_add_W: str = field(
        default="True",
        metadata={
            "help": "If add W when calculating lora ESD."
        },
    )
