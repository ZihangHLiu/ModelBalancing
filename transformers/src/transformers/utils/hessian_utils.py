...
class hessian():
    ...
    def __init__(self, model, criterion, one_input=False, data=None, dataloader=None, cuda=True, no_criterion=False, args=None):
        """
        ...
        self.no_criterion = no_criterion
        self.args = args
    ...
    def dataloader_hv_product(self, v):
        ...
        if self.one_input:
            if self.no_criterion:
                for batch in self.data:
                    batch = tuple(t.to(self.args.device) for t in batch)
                    inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": batch[3]}
                    tmp_num_data = len(inputs["labels"])
                ...
                for inputs in self.data:
                    ...
                    loss = self.criterion(self.model, inputs)
                    loss = loss.mean()
                    if "labels" in inputs.keys():
                        tmp_num_data = len(inputs["labels"])
                    else:
                        input_keys = list(inputs.keys())
                        tmp_num_data = len(inputs[input_keys[0]])
        else:
        ...
        # print(f"Total data num: {num_data}")
    ...
    def trace(self, maxIter=100, tol=1e-3):
        ...
        for i in range(maxIter):
            ...
            if self.full_dataset:
                ...
                Hv = hessian_vector_product(self.gradsH, self.params, v)
            tmp_layer_traces = np.array([torch.sum(Hv_ * v_).cpu().item() for Hv_, v_ in zip(Hv, v)])
            layer_traces.append(tmp_layer_traces) 

            trace_vhv.append(group_product(Hv, v).cpu().item())
            print(f"Iteration {i} hessian trace: {np.mean(trace_vhv)}")
            if abs(np.mean(trace_vhv) - trace) / (trace + 1e-6) < tol:
                break
            else:
                trace = np.mean(trace_vhv)
        print("Layer trace")
        print(np.mean(np.array(layer_traces)))
        print("trace")
        print(trace_vhv)
        return np.mean(np.array(layer_traces), axis=0), trace_vhv
